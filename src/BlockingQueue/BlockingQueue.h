...
namespace my_tt {
	template<typename items_type, typename base_type>
	class BlockingQueue {
	public:
		...
		~BlockingQueue();
	private:
		std::queue<items_type, base_type
> bq;
	...
	template<typename items_type, typename base_type>
	BlockingQueue<items_type, base_type>::BlockingQueue(const int &n, items_type x)
	...
	template<typename items_type, typename base_type>
	BlockingQueue<items_type, base_type>::BlockingQueue(std::initializer_list<items_type> lst)
		:limits(std::numeric_limits<int>::max()) {
		for ( auto x : lst ) {
			this->bq.push(x);
		}
	...
	template<typename items_type, typename base_type>
	int BlockingQueue<items_type, base_type>::size() {
		std::unique_lock<std::mutex> lock(this->add_query_mutex);
		return bq.size();
	}
	template<typename items_type, typename base_type>
	bool BlockingQueue<items_type, base_type>::empty() {
		std::unique_lock<std::mutex> lock(this->add_query_mutex);
		return bq.empty();
	}
	template<typename items_type, typename base_type>
	items_type BlockingQueue<items_type, base_type>::back() {
		std::unique_lock<std::mutex> lock(this->add_query_mutex);
		if ( this->bq.empty() )
	...
	template<typename items_type, typename base_type>
	items_type BlockingQueue<items_type, base_type>::front() {
		std::unique_lock<std::mutex> lock(this->add_query_mutex);
		if ( this->bq.empty() )
			throw std::runtime_error("Queue is empty.");
		return bq.front();
	...
	template<typename items_type, typename base_type>
	void BlockingQueue<items_type, base_type>::add(items_type item) {
		std::unique_lock<std::mutex> lock(this->add_query_mutex);
	...
	template<typename items_type, typename base_type>
	void BlockingQueue<items_type, base_type>::remove() {
	...
	template<typename items_type, typename base_type>
	BlockingQueue<items_type, base_type>::~BlockingQueue() {
		std::unique_lock<std::mutex> lock(this->del_mutex);
		while ( !this->bq.empty() ) {
			this->bq.pop();
		}
